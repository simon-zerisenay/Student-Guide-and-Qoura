...
jQuery.widget( "mobile.scrollview", jQuery.mobile.widget, {
	options: {
		...
		scrollMethod:      /webkit/.test(navigator.userAgent.toLowerCase())?"translate":"position",  // "translate", "position", "scroll"
	...
	_handleDragStart: function(e, ex, ey)
	{
		...
		if (this.options.eventType == "mouse" || this.options.delayedClickEnabled) {
			//To fix ETST-7254 added workaround for <input ...> elements
			//To fixETST-8233 added textarea 
			if(e.srcElement && e.srcElement.tagName != "INPUT" && e.srcElement.tagName != "TEXTAREA") {
				e.preventDefault();
			}
		}
		//To fix ETST-8234 added workaround for <span ...> elements
		if (e.srcElement && e.srcElement.tagName != "SPAN") {
			e.stopPropagation();
		}
	},
